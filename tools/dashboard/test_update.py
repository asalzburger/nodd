...
class UpdatePolicyTests(unittest.TestCase):
    def test_project_pr_requires_record_update(self):
        with self.assertRaisesRegex(ValueError, 'must update'):
            check_update.check('Global: Define synthetic architecture', ['docs/design/TEST.md'])
        self.assertIn('includes a tracking update', check_update.check(
            'Global: Define synthetic architecture', ['docs/design/TEST.md','project/tracking.json']))
        self.assertIn('includes a tracking update', check_update.check(
            'Tracker: Review synthetic design', ['project/reviews.json']))

    def test_allowed_areas_and_exact_title_format(self):
        for area in check_update.PR_AREAS:
            with self.subTest(area=area):
                paths = ['.github/workflows/TEST.yml'] if area == 'Infrastructure' else ['project/tracking.json']
                check_update.check(area + ': Synthetic change', paths)
        for title in ('chore: cleanup', 'chore(ci): cleanup', 'Tracker:',
                      'Tracker: ', 'Tracker:  double space', 'tracker: change',
                      'Global:change', 'Other: change', 'Software: a\nb',
                      'Unprefixed change'):
    ...
    def test_historical_maintenance_classification(self):
        ...
        for title in ('chore-ish change', 'chore:', 'choreography: new design', 'Software: tool'):
    ...
    def test_metadata_correction_does_not_track_chore(self):
        self.assertIn('excluded',check_update.check('Infrastructure: correct synthetic metadata',
    ...
    def test_cli_uses_real_git_diff_not_only_title(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            data={'pull_request':{'title':'Software: Synthetic project PR','base':{'sha':base},
            ...
            data['pull_request']['title']='Infrastructure: synthetic maintenance'
            event.write_text(json.dumps(data))
