...
def main():
    ...
    def draw(ax, option, regions):
        ...
        end = regions['muon_endcap']
        ax.set_title(f"{option['id']} · {option['title']}\n"
                     f"Barrel r {host['r_min_m']:g}–{host['r_max_m']:g}, |z| ≤ {host['z_max_m']:g} m\n"
                     f"Endcap r {end['r_min_m']:g}–{end['r_max_m']:g}, |z| {end['z_min_m']:g}–{end['z_max_m']:g} m",fontsize=9,pad=8)
    ...
    footer=('DRAFT / PROTOTYPE · candidate composite hosts include magnets, chambers, supports and routes; not fully sensitive volumes\n'
            ...
            'E1-R2 baseline stays unchanged. Forward calorimetry remains at |z| = 11.2–13.2 m; the 0.30 m gap needs an interface study.')
    ...
    for ax, option in zip(axes.flat,config['options']):
        ...
        save(single,stem)
        e = regions['muon_endcap']; b = regions['muon_barrel']
        row={'candidate':option['id'],'allocations':[{k:r[k] for k in ('id','r_min_m','r_max_m','z_min_m','z_max_m')} for r in regions.values()],
             ...
             'endcap_axial_depth_m':e['z_max_m']-e['z_min_m'],
             'endcap_front_ray_radius_m':{str(eta):e['z_min_m']/math.sinh(eta) for eta in (3.,3.5)},
